...
const DashboardJoueur = dynamic<{ teamId: string }>(
...
export default function DashboardIndex() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">Vérification de votre session...</div>
      ...
      <div className="space-y-4">
        ...
        {roles.admin && (
          <div className="border border-gray-700 rounded overflow-hidden">
            <button className="w-full text-left p-4 bg-gray-800 hover:bg-gray-700 font-bold"
        ...
        {(roles.club_admin || roles.admin) && (
          <div className="border border-gray-700 rounded overflow-hidden">
            ...
            <div className={`transition-all duration-500 overflow-hidden ${openPanel === "club" ? "max-h-[5000px]" : "max-h-0"}`}>
              {openPanel === "club" && <DashboardClubAdmin roles={{ admin: roles.admin, club_admin: roles.club_admin }} clubMemberships={clubMemberships} />}
            </div>
          </div>
        )}
            ...
            return (
              <div key={panelKey} className="border border-gray-700 rounded overflow-hidden">
                ...
                <div className={`transition-all duration-500 overflow-hidden ${openPanel === panelKey ? "max-h-[5000px]" : "max-h-0"}`}>
                  {openPanel === panelKey && <div className="p-4"><DashboardJoueur teamId={t.team_id} /></div>}
                </div>
              </div>
            )
          })}
