...
function ExpandableItem({ title, isOpen, onToggle, children }: ExpandableItemProps) {
  ...
  return (
    <div className="mb-4 border-b border-gray-600">
      ...
      >
        {/* ✅ On ne rend le contenu que si ouvert */}
        {isOpen && children}
      </div>
    </div>
  )
              ...
              <div className={`mt-2 bg-gray-800 rounded shadow ${isOpen ? 'p-4 md:p-6' : 'p-0'}`}>
                {key === 'clubs' && clubs.map(club => (
                  ...
                  >
                    <ClubForm
                      clubId={club.id}
                      onSaved={async () => { await fetchClubs(); setOpenClubId(null) }}
                ...
                {key === 'teams' && teams.map(team => (
                  ...
                  >
                    <TeamForm
                      teamId={team.id}
                      onSaved={async () => { await fetchTeams(); setOpenTeamId(null) }}
                ...
                {key === 'users' && users.map(user => (
                  ...
                  >
                    <EditUser
                      userId={user.id}
                      onSaved={async () => { await fetchUsers(); setOpenUserId(null) }}
