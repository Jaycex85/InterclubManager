...
function ExpandableItem({ title, isOpen, onToggle, children }: ExpandableItemProps) {
  ...
  // on attend que le composant soit monté pour calculer scrollHeight
  ...
  }, [isOpen, children, mounted])

  return (
    <div className="mb-4 border-b border-gray-600">
      <button className="w-full text-left font-bold p-2 bg-gray-700 hover:bg-gray-600" onClick={onToggle}>
        {title}
      </button>
      <div ref={ref} style={{ maxHeight: mounted ? height : '0px', overflow: 'hidden', transition: 'max-height 0.35s ease' }} className="p-2 bg-gray-600 rounded mt-2">
        {children}
      </div>
    </div>
  )
...
export default function AdminDashboard() {
  ...
  const togglePanel = (panel: PanelKey) => setOpenPanels(prev => prev.includes(panel) ? prev.filter(p => p !== panel) : [...prev, panel])

  const fetchClubs = async () => { const { data, error } = await supabase.from('clubs').select('*').order('name'); if (!error) setClubs(data || []) }
  const fetchTeams = async () => {
    ...
    `).order('name')
    if (!error) setTeams((data || []).map((t: any) => ({
      id: t.id, name: t.name, club_id: t.club_id,
      club_name: t.club?.name || '',
      category: t.category, captain_id: t.captain_id,
      captain_email: t.captain?.email || ''
    })))
  }
  const fetchUsers = async () => { const { data, error } = await supabase.from('users').select('*').order('email'); if (!error) setUsers(data || []) }

  useEffect(() => { fetchClubs(); fetchTeams(); fetchUsers() }, [])
            ...
            <div key={key} ref={panelRefs[key]}>
              ...
              <div className={`mt-2 bg-gray-800 rounded shadow ${isOpen ? 'p-4 md:p-6' : 'p-0'}`}>
                {key === 'clubs' && clubs.map(club => (
                  ...
                  >
                    <ClubForm clubId={club.id} onSaved={async () => { await fetchClubs(); setOpenClubId(null) }} onClose={() => setOpenClubId(null)} />
                  </ExpandableItem>
                ))}
                {key === 'teams' && teams.map(team => (
                  ...
                  >
                    <TeamForm teamId={team.id} onSaved={async () => { await fetchTeams(); setOpenTeamId(null) }} onClose={() => setOpenTeamId(null)} />
                  </ExpandableItem>
                ))}
                {key === 'users' && users.map(user => (
                  ...
                  >
                    <EditUser userId={user.id} onSaved={async () => { await fetchUsers(); setOpenUserId(null) }} onClose={() => setOpenUserId(null)} />
                  </ExpandableItem>
                ))}
              </div>
            </div>
